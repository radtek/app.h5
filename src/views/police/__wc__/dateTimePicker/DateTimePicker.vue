    ...
    <transition name="select">
      <div class="picker-box"
        ...
        <div class="picker-box-header">
          <div class="picker-box-header-cancle"
               @click="show = false">{{cancel}}</div>
          {{title}}
          <div class="picker-box-header-confirm"
               @click="submit">{{confirm}}</div>
        </div>
        <div class="picker-box-content">
          <div class="picker-box-content-list"
               v-if="type === 'dateTime' || type === 'date'">
            <ul :class="{'is_dragging': yearState.dragging}"
          ...
          <div class="picker-box-content-list"
               v-if="type === 'dateTime' || type === 'date'">
            <ul :class="{'is_dragging': monthState.dragging}"
              ...
              <li></li>
              <li></li>
              <li v-for="(item, index) in monthState.data"
                  :key="index"
                  :class="{
          ...
          <div class="picker-box-content-list"
            ...
            <ul ref:area-list
              ...
              <li></li>
              <li></li>
              <li v-for="(item, index) in dayState.data"
                  :key="index"
                  :class="{
          ...
          <div class="picker-box-content-list"
            ...
            <ul :class="{'is_dragging': hourState.dragging}"
          ...
          <div class="picker-box-content-list"
               v-if="type === 'dateTime' || type === 'time'">
            <ul :class="{'is_dragging': minuteState.dragging}"
              ...
              <li></li>
              <li></li>
              <li v-for="(item, index) in minuteState.data"
                  :key="index"
                  :class="{
  ...
  export default {
  	...
  	methods: {
  		...
  		startControlComputed() {
  			const startControlResult = {};
  			startControlResult.yearFlag =
  				this.yearState.selectedId === this.startYear;
  			startControlResult.monthFlag =
  				this.monthState.selectedId === this.startMonth;
  			return startControlResult;
  		},
  		endControlComputed() {
  			const endControlResult = {};
  			endControlResult.yearFlag =
  				this.yearState.selectedId === this.endYear;
  			endControlResult.monthFlag =
  				this.monthState.selectedId === this.endMonth;
  		...
  		initNowData() {
  			...
  			hourState.selectedId =
  				curDate.getHours() < 10
  					? "0" + curDate.getHours()
  					: curDate.getHours();

  			minuteState.data = this.computedMinute();
  			minuteState.selectedId =
  				...
  				setTimeout(
  					() => {
  						...
  						for (let i = 0; i < minuteState.data.length; i++) {
  							if (
  								minuteState.selectedId === minuteState.data[i]
  							) {
  								minuteState.index = i;
  							}
  						}
  		...
  		transToMonthFilter() {
  			...
  			if (
  				this.monthState.data[monthLength - 1] <
  					this.monthState.selectedId ||
  				this.monthState.data[0] > this.monthState.selectedId
  			) {
  		...
  		transToDayFilter() {
  			...
  			if (
  				this.dayState.data[dayLength - 1] < this.dayState.selectedId ||
  				this.dayState.data[0] > this.dayState.selectedId
  			) {
  		...
  		getSelectedData(index) {
  			...
  			const thisData = this[target + "State"];
  		...
  		setPage() {
  			const target = this.target;
  			const thisData = this[target + "State"];
  			const clientHeight = this.slideEls[0].getBoundingClientRect()
  				.height;
  			const total = thisData.data.length;
  			let goPage = Math.round(
  				(thisData.translateY / clientHeight).toFixed(1)
  			);
  			if (goPage > 0) {
  				goPage = 0;
  			}
  			goPage = total - 1 >= Math.abs(goPage) ? goPage : -(total - 1);
  			this.getSelectedData((thisData.index = Math.abs(goPage)));

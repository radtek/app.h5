              ...
              <template v-else>
                <template v-for="(option,index) in theme.infoVoteOptionsList">
                  <textarea style="width:100%;"
                            ...
                            :disabled="status"
                            v-model="option.text"
                            placeholder="请输入答案(不超过250个字)"
                            @change="onTextChange(option)"></textarea>
                </template>
              </template>
            </div>
          </div>
  ...
  export default {
  	...
  	methods: {
  		__calcYXCount() {
  			...
  			if (parseInt(count * 3, 10) > vote.infoVoteThemeList.length) {
  				this.$alert(
  		...
  		onTextChange(option) {
  			if (option && option.text && option.text.length > 250) {
  				option.text = option.text.substring(0, 250);
  			}
  		},
  		...
  		async __fetch() {
  			...
  			if (this.isVoted && !this.$isDev) {
  				JXRSApi.app.h5.voteOK();
  			}
  		...
  		submit() {
  			...
  			if (this.mzpy === "1") {
  				if (
  					parseInt(mzpyYXList.length * 3, 10) > voteResultList.length
  				) {
  					this.$alert(
  						"您所投的优秀党员票数超额，请您修改互评投票结果",
  						"提示",

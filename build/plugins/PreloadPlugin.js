				...
				htmlPluginData => {
					...
					extractedChunks = extractedChunks.filter(chunk =>
						doesChunkBelongToHTML(
							chunk,
							getValues(htmlPluginData.assets.chunks),
							{}
						));

					let files = flatten(
						extractedChunks.map(chunk => chunk.files)
					);
							...
							if (options.rel === "preload") {
								...
								const crossOrigin =
									asValue === "font"
										...
										: "";
								filesToInclude += `<link rel="${
									options.rel
								}" as="${asValue}" ${crossOrigin}href="${entry}">\n`;
							} else {

...
<template>
	<div class="rx-swipeout-item"
	     ...
	     @mouseup="end"
	     @touchcancel="end">
		<div class="rx-swipeout-button-box rx-swipeout-button-box-left"
		     :style="leftButtonBoxStyle"
		     v-show="distX >= 0">
			<slot name="left-menu"></slot>
		</div>
		<div class="rx-swipeout-button-box"
		     :style="rightButtonBoxStyle"
		     v-show="distX <= 0">
			<slot name="right-menu"></slot>
		</div>
		<div class="rx-swipeout-content"
		     :style="styles"
	...
	export default {
		...
		methods: {
			...
			caculateMenuWidth(direction) {
				const list = this.$slots[`${direction}-menu`][0].children.filter(
					one => one.tag
				);
				let width = 0;
				list.forEach(one => {
					const propsData = one.componentOptions
						? one.componentOptions.propsData
						: {};
			...
			start(ev) {
				...
				if (this.$parent.$options._componentTag === "swipeout") {
					const openItems = this.$parent.$children.filter(
						item =>
							item.$data.styles.transform.indexOf("(0px, 0, 0)") ===
							-1
					);
			...
			move(ev) {
				...
				if (this.valid === undefined) {
					if (this.distX > 0 && this.hasLeftMenu === false) {
						...
						Math.abs(this.distX) > this.sensitivity ||
						Math.abs(this.distY) > this.sensitivity
					) {
				...
				if (this.valid === true) {
					if (Math.abs(this.distX) <= this.menuWidth) {
						...
						const offset =
							(this.menuWidth + extra) * (this.distX < 0 ? -1 : 1);
				...
				if (this.valid === true) {
					if (this.distX < 0 && this.direction === "right") {
						const threshold =
							this.threshold <= 1
								? this.rightMenuWidth * this.threshold
								: this.threshold;
						...
						const threshold =
							this.threshold <= 1
								? this.leftMenuWidth * this.threshold
								: this.threshold;
						if (this.distX > threshold) {
			...
			setOffset(x, animated = false, force) {
				...
				if (
					(this.direction === "right" && x > 0) ||
					(this.direction === "left" && x < 0)
				) {
					x = 0;
				}
				if (x === 0) {
		...
		computed: {
			...
			rightButtonBoxStyle() {
				...
				if (this.transitionMode === "follow") {
					let offset =
						this.styles.transform.indexOf("(0px, 0, 0)") === -1
							? this.rightMenuWidth - Math.abs(this.distX)
							: this.rightMenuWidth;
					if (offset < 0) {
						offset = 0;
					}
